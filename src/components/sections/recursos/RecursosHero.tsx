...
const SUBTITULO =
  'Legislação federal e estadual, cartilhas oficiais do MDH, modelos de documentos e guias técnicos — reunidos em um só lugar para apoiar gestores, conselheiros e técnicos municipais.';
...
export default function RecursosHero() {
  ...
  const destaqueRef = useRef<HTMLSpanElement | null>(null);
  const subRef      = useRef<HTMLParagraphElement | null>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      // Word-wrap headline, preserving the highlight span
      const textSpan = headlineRef.current?.querySelector(
        '.headline-text',
      ) as HTMLElement | null;
      if (textSpan) wrapWordsPreservingMarkup(textSpan, 'rec-hero-word');

      // Initial states
      gsap.set(labelRef.current,   { y: 20, autoAlpha: 0 });
      gsap.set('.rec-hero-word',   { y: 30, autoAlpha: 0 });
      gsap.set(destaqueRef.current, { autoAlpha: 0 });
      ...
      tl.to(overlayRef.current, { autoAlpha: 0, duration: 0.6, ease: 'power2.out' })
        ...
        .to('.rec-hero-word', { y: 0, autoAlpha: 1, duration: 0.85, stagger: 0.05 }, 0.7)
        .to(destaqueRef.current, { autoAlpha: 1, duration: 0.6, ease: 'power2.out' }, 1.3)
  ...
  return (
    ...
    >
      {/* Entrance overlay — fades out on load */}
      <div
        ref={overlayRef}
        aria-hidden="true"
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 500px 350px at center, rgba(40,168,122,0.10), transparent 70%)',
        }}
      />
      ...
      <div className="container-x relative">
        <div className="mx-auto max-w-3xl text-center">
          <span ref={labelRef} className="inline-block">
            <SectionLabel>Biblioteca de Recursos Educativos</SectionLabel>
          </span>

          <h1
            ref={headlineRef}
            id="recursos-hero-heading"
          ...
          >
            ...
            >
              implementar o Fundo Municipal
            </span>
          </h1>

          <p
            ref={subRef}
            className="mt-7 text-[17px] sm:text-[18px] leading-[1.7] text-text-secondary mx-auto max-w-2xl"
          >
            {SUBTITULO}
          </p>
        </div>
